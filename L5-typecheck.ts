...
export const typeofExp = (exp: Parsed | Error, tenv: TEnv): TExp | Error =>
    ...
    isDefineExp(exp) ? typeofDefine(exp, makeExtendTEnv([(<DefineExp>exp).var.var],[(<DefineExp>exp).var.texp],tenv)) :
    isProgram(exp) ? typeofProgram(exp, tenv) :
    // Skip isSetExp(exp) isLitExp(exp)
    Error("Unknown type"+ JSON.stringify(exp,null,2));
...
export const typeofExps = (exps: Exp[], tenv: TEnv): TExp | Error =>
    ...
    isDefineExp(first(exps)) ? typeofDefineExps(exps,tenv) :
...
// Typing rule:
//   (define (var : texp) val)
//   tenv-new=extend-tenv(var=texp ; tenv)
//   If   type<var>(tenv-new) = t1  
//        type<val>(tenv-new) = t1
//   then type<(define (var : texp) val)> = void
//   else type<(define (var : texp) val)> = Error
//
//
//   ** We assume that tenv is already extended with var's declared type **
export const typeofDefine = (exp: DefineExp, tenv: TEnv): TExp | Error => {
    //type of exp.val should be same as ex.var.texp
    const constraint=checkEqualType(typeofExp(exp.val,tenv),exp.var.texp,exp);
    if(isError(constraint))
        return constraint;
    else
        return makeVoidTExp();
};
// Type of sequence of expressions, when the first one is a DefineExp.
// Type Check it and compute the type of the rest, with the updated Type Environment.
export const typeofDefineExps=(exps: Exp[], tenv: TEnv) : TExp| Error =>{
    let first_define = typeofDefine(first(exps),tenv);
    if (isError(first_define))
        return first_define;
    else
        return typeofExps(rest(exps),makeExtendTEnv([(first(exps)).var.var],[(first(exps)).var.texp],tenv));
}

// Purpose: compute the type of a program
// Typing rule:
// TODO - write the true definition
// If type <exp_i>(tenv) = t_i for i = 1 to n
// Then type<exp>(tenv)  = t_n

// return the Type of an expression sequence (Type of the last one)
